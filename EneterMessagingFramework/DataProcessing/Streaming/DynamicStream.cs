    ...
    public class DynamicStream : Stream
    {
        ...
        public override int Read(byte[] buffer, int offset, int count)
        {
            using (EneterTrace.Entering())
            {
                ...
                lock (myLockDuringReading)
                {
                    ...
                    // While we have not read the requested amount of data yet.
                    while (aReadSize < count && !myIsClosed)
                    {
                        // Remove the sequence of bytes from the queue.
                        object aDataObject = myMessageQueue.DequeueMessage();

                        if (aDataObject != null)
                        {
                            // Put the sequence of bytes to the memory stream.
                            byte[] aData = (byte[])aDataObject;
                            myDataInProgress = new MemoryStream(aData);
        ...
        public override void Write(byte[] buffer, int offset, int count)
        {
            using (EneterTrace.Entering())
            {
                ...
                {
                    using (MemoryStream aMemoryStream = new MemoryStream(buffer, offset, count))
                    {
                        byte[] aData = aMemoryStream.ToArray();
                        myMessageQueue.EnqueueMessage(aData);
                    }
                }
            }
        }
